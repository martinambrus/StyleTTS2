...
def build_asr_model(model_params: Optional[Dict[str, object]] = None, state_dict: Optional[Dict[str, torch.Tensor]] = None):
    """Instantiate the ASR model with sensible defaults."""

    params = dict(model_params or {})
    params.setdefault("input_dim", 80)
    params.setdefault("hidden_dim", 256)
    params.setdefault("token_embedding_dim", 256)
    params.setdefault("n_layers", 6)
    params.setdefault("location_kernel_size", 63)
    params.setdefault("attention_dropout", 0.0)
    params.setdefault("multi_task_config", params.get("multi_task_config", {}))
    params.setdefault("stabilization_config", params.get("stabilization_config", {}))
    params.setdefault("memory_optimization_config", params.get("memory_optimization_config", {}))

    return ASRCNN(**params)
...
class ASRCNN(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask=None, text_input=None, return_dict: bool = False):
        x = self.to_mfcc(x)
        x = self.init_cnn(x)
        ...
        if text_input is not None:
            return outputs.get("ctc_logits"), outputs.get("s2s_logits"), outputs.get("s2s_attn")

        return outputs.get("ctc_logits")
